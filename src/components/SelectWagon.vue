      ...
      <div class="col-5 d-flex">
        <b-form @submit.prevent="onWagonSelected" class="wagon-form" novalidate>
          ...
          <b-form-group id="wagon-code" label="Vagoneta Inicial" label-for="wagon-input">
            ...
            <span
              class="help-block error"
              v-if="!$v.startWagon.upperLimit"
            >El número máximo de vagoneta para este horno es {{ this.oven.wagons.length }}.</span>
          </b-form-group>
...
const upperLimit = oven => value => value <= oven.wagons.length;
...
export default {
  ...
  methods: {
    ...mapActions(["addStartingWagon"]),
    onWagonSelected() {
      this.addStartingWagon({
        oven: this.oven,
  ...
  validations() {
    return {
      startWagon: {
        positiveInteger,
        upperLimit: upperLimit(this.oven),
      },
      form: ["startWagon"]
    };
  }
